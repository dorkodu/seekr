...
/**
 * Simple wrapper class for common utils that we use at Dorkodu, in our PHP code.
 *
...
class Utils
{
   ...
   * works just like @{function:Utils::indexPull}, except that it operates on a list of
   ...
   *    }
   *
   * You can express this more concisely with Utils::methodPull():
   *
   *    $names = Utils::methodPull($objects, 'getName');
   *
   * Utils::methodPull() takes a third argument, which allows you to do the same but for
   * the array's keys:
   *
   *    COUNTEREXAMPLE
   *    $names = array();
   ...
   *    }
   *
   * This is the Utils::methodPull version():
   *
   *    $names = Utils::methodPull($objects, 'getName', 'getID');
   *
   ...
   *    }
   *
   * With Utils::methodPull():
   *
   *    $id_map = Utils::methodPull($objects, null, 'getID');
   *
   * See also @{function:Utils::indexPull}, which works similarly but accesses array indexes
   ...
   */
  public static function methodPull(array $list, $method, $key_method = null)
  {
    $result = array();
   ...
   * function works just like @{function:Utils::methodPull}, except that it accesses object
   ...
   *    }
   *
   * You can express this more concisely with Utils::propertyPull():
   *
   *    $names = Utils::propertyPull($objects, 'name');
   *
   * Utils::propertyPull() takes a third argument, which allows you to do the same but for
   * the array's keys:
   *
   *    COUNTEREXAMPLE
   *    $names = array();
   ...
   *    }
   *
   * This is the Utils::propertyPull version():
   *
   *    $names = Utils::propertyPull($objects, 'name', 'id');
   *
   * If you pass ##null## as the second argument, the objects will be preserved:
   *
   *    COUNTEREXAMPLE
   ...
   *    }
   *
   * With Utils::propertyPull():
   *
   *    $id_map = Utils::propertyPull($objects, null, 'id');
   *
   * See also @{function:Utils::methodPull}, which works similarly but calls object methods
   ...
   */
  public static function propertyPull(array $list, $property, $key_property = null)
  {
    $result = array();
    foreach ($list as $key => $object) {
   ...
   * works just like @{function:Utils::methodPull}, except that it operates on a list of
   ...
   *    }
   *
   * With Utils::indexPull():
   *
   *    $names = Utils::indexPull($list, 'name');
   *
   * See @{function:Utils::methodPull} for more usage examples.
   *
   ...
   */
  public static function indexPull(array $list, $index, $key_index = null)
  {
    $result = array();
   ...
   *    }
   *
   * This can be expressed more tersely with Utils::methodGroup():
   *
   *    $animals_by_species = Utils::methodGroup($animals, 'getSpecies');
   *
   ...
   *
   * See also @{function:Utils::indexGroup}, which works the same way but operates on
   * array indexes.
   *
   ...
   */
  public static function methodGroup(array $list, $by /* , ... */)
  {
    $map = Utils::methodPull($list, $by);
    ...
    if ($args) {
      ...
      foreach ($groups as $group_key => $grouped) {
        $args[0] = $grouped;
        $groups[$group_key] = call_user_func_array('Utils::methodGroup', $args);
      }
    }
   ...
   */
  public static function indexGroup(array $list, $by /* , ... */)
  {
    $map = Utils::indexPull($list, $by);
    ...
    if ($args) {
      ...
      foreach ($groups as $group_key => $grouped) {
        $args[0] = $grouped;
        $groups[$group_key] = call_user_func_array('Utils::indexGroup', $args);
      }
    }
   ...
   *    $sorted = Utils::methodSort($ducks, 'getName');
   *
   * It is usually significantly more efficient to define an ordering method
   * on objects and call ##Utils::methodSort()## than to write a comparator. It is often more
   * convenient, as well.
   *
   ...
   */
  public static function methodSort(array $list, $method)
  {
    $surrogate = Utils::methodPull($list, $method);
   ...
   */
  public static function indexSort(array $list, $index)
  {
    $surrogate = Utils::indexPull($list, $index);
   ...
   * @{function:Utils::indexFilter}, except that it operates on a list of objects instead
   * of a list of arrays.
   *
   ...
   * out Utils::nonEmpty objects. To remove all objects that DO have children, do this:
   *
   ...
   */
  public static function methodFilter(array $list, $method, $negate = false)
  {
    if (!is_string($method)) {
   ...
   *
   *   Utils::indexFilter($list, 'username');
   *
   * The optional third parameter allows you to negate the operation and filter
   * out Utils::nonEmpty arrays. To remove all arrays that DO have value for key
   * 'username', do this:
   *
   *   Utils::indexFilter($list, 'username', true);
   *
   ...
   */
  public static function indexFilter(array $list, $index, $negate = false)
  {
    if (!is_scalar($index)) {
   ...
   */
  public static function assertInstancesOf(array $arr, $class)
  {
   ...
   * @task   assert
   */
  public static function assertStringLike($parameter)
  {
   ...
   * non-##null##. If no argument is Utils::nonEmpty, it returns the last argument. This
   * is useful idiomatically for setting defaults:
   *
   *   $display_name = Utils::nonEmpty($user_name, $full_name, "Anonymous");
   *
   * @param  ...         Zero or more arguments of any type.
   ...
   */
  public static function headKey(array $arr)
  {
    reset($arr);
    return key($arr);
  }
   ...
   */
  public static function arrayMergeV(array $arrayv)
  {
    if (!$arrayv) {
      return array();
    }
    ...
    foreach ($arrayv as $key => $item) {
      if (!is_array($item)) {
        throw new InvalidArgumentException(
          sprintf(
            'Expected all items passed to Utils::arrayMergeV() to be arrays, but ' .
   ...
   */
  public static function splitLines($corpus, $retain_endings = true)
  {
    if (!strlen($corpus)) {
      return array('');
    }
   ...
   */
  public static function arrayFuse(array $list)
  {
    if ($list) {
   ...
   */
  public static function arrayInterleave($interleave, array $array)
  {
    $result = array();
   ...
   * @group library
   */
  public static function isHiphopRuntime()
  {
   ...
   */
  public static function loggableString($string)
  {
   ...
   */
  public static function fwrite_nonblockingStream($stream, $bytes)
  {
    if (!strlen($bytes)) {
      return 0;
    }
   ...
   *                    failed to decode.
   */
  public static function jsonDecode($string, $default = array())
  {
   ...
   */
  public static function censorCredentials($string)
  {
